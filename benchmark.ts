...
const runGroq = async (audioUrl: string) => {
...
const runJigsawStack = async (audioUrl: string) => {
  await jigsawstack.audio.speech_to_text({
    url: audioUrl,
  });
};

const runAssemblyAI = async (audioUrl: string) => {
...
const stt = async (
  ...
  await func(audioUrl).catch((error) => {
...
const benchmark = async () => {
  ...
  for (
    ...
    const sampleKey = `sample_${audioSampleIndex + 1}`;
